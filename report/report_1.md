...
### Uvod (Introduction)
Natjecanje za pjesmu Eurovizije (ESC) predstavlja složen sociokulturni i estetski fenomen u kojem na ishod utječe mnoštvo varijabli — od geopolitičkih saveza do suvremenih produkcijskih trendova. Tradicionalne metode analize često se fokusiraju na izolirane varijable (npr. samo bodove ili samo žanr). Svrha projekta "ESC Mreža" bila je razviti nelinearni sustav koji tretira svaku pjesmu kao čvorište (node) povezano s nizom faktora (attributes), omogućujući istraživačima i korisnicima uočavanje klastera (clusters) uspjeha.
